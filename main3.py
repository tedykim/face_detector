###########################  웹캠으로 얼굴인식하고, lyan으로 overlay처리하기(scaler를 작게하면 인식잘안됨)
import cv2
import dlib
import sys
import numpy as np
...
def overlay_transparent(background_img, img_to_overlay_t, x, y, overlay_size=None):
    ...
    if overlay_size is not None:
        # Check the datatype of img_to_overlay_t
...
# Use webcam
cap = cv2.VideoCapture(0)  # Changed to webcam (index 0)

# loop
while True:
    # read frame buffer from webcam
    ret, img = cap.read()
    if not ret:
        break
    ...
    if 'result' in locals():  # Check if 'result' variable is defined
...
# # cv2.VideoCapture(0) : 파일이름대신 '0' 을 넣으면, 웹캠이 켜짐
# scaler = 0.3
